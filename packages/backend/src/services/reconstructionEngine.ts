import { CorrelationCluster } from './correlationEngine';
...
export const runReconstruction = (caseId: string, clusters: CorrelationCluster[]): Reconstruction[] => {
  const events = getRows("SELECT * FROM events WHERE case_id = ?", [caseId]);
  const reconstructions: Reconstruction[] = [];
  let reconIdx = 1;

  const createRecon = (name: string, description: string, evts: any[], cls: any, conf: number, customNotes: Record<string, string> = {}) => {
    const steps: ReconstructionStep[] = evts.map(e => ({
