import { runQuery, getRows } from './db';

export const seedDemoCase = async () => {
  const existing = getRows("SELECT id FROM cases WHERE id = 'CASE-2026-014'");
  ...
  for (const e of evidence) {
    runQuery("INSERT INTO evidence (id, case_id, filename, file_type, size, hash, upload_timestamp, source, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", [
...
80. EVT-080 10:10:05 AUTH LOGOUT analyst01 WS-104 confidence:0.99 FACT ev:EV-002
  ...
  for (const line of lines) {
    ...
    if (mapped) srcType = mapped.src;

    runQuery("INSERT INTO events (id, case_id, timestamp, timestamp_confidence, source_type, event_type, actor, description, confidence, evidence_refs) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [
  ...
  for (const ent of entities) {
    runQuery("INSERT INTO entities (id, case_id, type, value, first_seen, last_seen) VALUES (?, ?, ?, ?, ?, ?)", [
  ...
  for (const f of findings) {
    runQuery("INSERT INTO findings (id, case_id, description, classification, confidence, evidence_refs, status, supporting, contradicting) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", [
  ...
  for (const q of questions) {
    runQuery("INSERT INTO questions (id, case_id, text, answer, confidence, evidence_refs, missing_evidence) VALUES (?, ?, ?, ?, ?, ?, ?)", [
