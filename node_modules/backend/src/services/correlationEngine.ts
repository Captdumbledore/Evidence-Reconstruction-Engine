...
export const runCorrelation = (caseId: string): CorrelationCluster[] => {
  ...
  events.forEach(e => {
    if (!actorMap[e.actor]) actorMap[e.actor] = [];
  ...
  for (let i = 0; i < events.length; i++) {
    if (events[i].event_type === 'EMAIL_RECEIVED') {
      ...
      let conf = 0.30;
      let missing = 0;
      if (events[i]) conf += 0.15; else missing++;
      if (hasWeb) { chain.push(hasWeb); conf += 0.15; } else missing++;
      if (hasDl) { chain.push(hasDl); conf += 0.15; } else missing++;
      if (hasFc && hasDl && new Date(hasFc.timestamp).getTime() - new Date(hasDl.timestamp).getTime() <= 120000) { chain.push(hasFc); conf += 0.10; }
      else if (hasFc) { chain.push(hasFc); conf += 0.10; } else missing++;
      if (hasPs && hasFc && new Date(hasPs.timestamp).getTime() - new Date(hasFc.timestamp).getTime() <= 180000) { chain.push(hasPs); conf += 0.10; }
      else if (hasPs) { chain.push(hasPs); conf += 0.10; } else missing++;
      if (hasNc && hasPs && new Date(hasNc.timestamp).getTime() - new Date(hasPs.timestamp).getTime() <= 300000) { chain.push(hasNc); conf += 0.15; }
      else if (hasNc) { chain.push(hasNc); conf += 0.15; } else missing++;
